...
void fastPwm3_ini(int freq)
{
    ...
    TCCR3B |= n; // Añadimos el prescale

    sei();
}

void fastPwm3_gen(int pin, double percent)
{
    // Convertimos grado en porcentaje de valor.
    unsigned int value = (int)(((double)TOP_ + 1.) * ((double)percent / 100.));
...
void loop()
{
    ...
    fastPwm3_ini(24000);
    fastPwm3_gen(5, 30); // OC3A, 30% TOP
    fastPwm3_gen(2, 60); // OC3B, 60% TOP
    fastPwm3_gen(3, 80); // OC3C, 80% TOP
